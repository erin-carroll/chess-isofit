...
surface_path = os.path.join(home, 'test_roi', 'surface_20240103_avirii_20250730.mat')
lut_config_path = os.path.join(home, 'test_roi', 'lut_config_20250810.json')
working_dir = os.path.join(home, 'test_flightlines', f'{flight_id}_sRTMnet_{version}')
wavelength_path = os.path.join(raw_dir, day, f'{flight_id}_rdn_ort.hdr')
fp_rcc = '/store/carroll/col/2018/test_roi/rcc_white_tarp_mean_20250814_v2.txt'

# run apply_oe to generate config, file structure, LUT
apply_oe(
        # file paths
        input_radiance = os.path.join(raw_dir, day, f'{flight_id}_rdn_ort'), # Radiance
        input_loc = os.path.join(raw_dir, day, f'{flight_id}_rdn_ort_igm_ort'), # Location - IGM (lon, lat, elev)
        input_obs = os.path.join(raw_dir, day, f'{flight_id}_rdn_obs_ort'), # Observations
        working_directory = os.path.join(working_dir), # Output directory
        surface_path = surface_path, # Surface priors (.mat file)
        ...
        model_discrepancy_path = None, # extra_data + 'avirisng_model_discrepancy_neon.mat -- this is what was blowing the final invert_analytical step to generate wild outputs
        inversion_windows = [[400.0, 1360.0], [1410, 1800.0], [1970, 2450.0]], # widened these from best practices documentation
        ...
        prebuilt_lut = None, # Use this pre-constructed look up table for all retrievals. Must be an ISOFIT-compatible RTE NetCDFs
