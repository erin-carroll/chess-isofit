...
ul_lr_list="/store/carroll/col/data/2025/mosaic/file_lists/ul_lr_grids_${domain}_neon.txt"
all_obs_files="/store/carroll/col/data/2025/mosaic/file_lists/top_priority_obs_${domain}.txt"
...
while read -r line; do
    ...
    mosaic_glt_out="/store/carroll/col/data/2025/mosaic/glt_chunks/${domain}_2025_mosaic_glt_${id}.tif"
    if [[ -f "$mosaic_glt_out" ]]; then
        echo "Skipping existing: $mosaic_glt_out"
        continue
    fi

    log_file="/home/carroll/logs/mosaic_glt_${domain}_2025_${id}.log"
    deprioritize_file_list="/store/carroll/col/data/2025/mosaic/file_lists/deprioritize_obs.txt"
    cloud_shadow_mask_list="/store/carroll/col/data/2025/mosaic/file_lists/cloud_shadow_${domain}.txt"
    sbatch \
        --job-name="mosaic_2025_${domain}_${id}" \
        --nodes=1 \
        --cpus-per-task=64 \
        --partition=standard \
        --mem=300G \
