...
subset_dir = home+'aspen/subset/'
flight_id = glob(subset_dir+'/*rdn')[0].split('_rdn')[0].split('/')[-1]
version = '20250731'
working_dir = home+f'aspen/{flight_id}_sRTMnet_{version}_pressureElev/'
...
surface_model_path = os.path.join(home, 'surface_20240103_avirii_20250730.json')
output_surface_file = os.path.join(home, 'surface_20240103_avirii_20250730.mat')
lut_config_path = os.path.join(home, 'lut_config_20250731.json')
...
apply_oe(
        ...
        input_loc = os.path.join(subset_dir,f'{flight_id}_igm'), # Location - IGM (lon, lat, elev)
        input_obs = os.path.join(subset_dir,f'{flight_id}_obs'), # Observations
        ...
        pressure_elevation = True, # flag to retrieve elevation. Don't need to retrieve it because we have it at high resolution w DEM
